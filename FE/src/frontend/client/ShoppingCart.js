                                ...
                                {coupons.units.map((item) => (
                                    <div className="form-check" key={item.id}>
                                      <input
                                          ...
                                          disabled={item.minium_order_value > cartTotal}
                                          checked={selectedCoupon?.id === item.id} // Chỉ đánh dấu checkbox được chọn
                                          onChange={(e) =>
                                              setSelectedCoupon(e.target.checked ? item : null) // Hủy chọn khi checkbox được bỏ chọn
                                          }
                                      />
                                      <label className="form-check-label" htmlFor={`discount_${item.id}`}>
                                        <div className="font-weight-bold">{item.name_coupon}</div>
                                        <div>
                                          Giảm {item.discount_value}% trên tổng đơn hàng
                                        </div>
                                        <div>
                                          {item.minium_order_value > cartTotal ? (
                                              <p style={{ color: "red" }}>
                                                Đơn hàng chưa đạt giá trị tối thiểu {formatCurrency(item.minium_order_value)}
                                              </p>
                                          ) : (
                                              <p style={{ color: "green" }}>Bạn đủ điều kiện áp dụng mã giảm giá!</p>
                                          )}
                                        </div>
                                      </label>
                                    </div>
