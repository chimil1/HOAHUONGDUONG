...
// export const fetchAverageStart = () => {
//   return (dispatch) => {
//     dispatch({ type: "FETCH_PRODUCTS_REQUEST" });
//     return axios
//       .get(`http://localhost:8000/api/product/averageStart/`)
//       .then((response) => {
...
export const fetchReviewFind = () => {
  return (dispatch) => {
    ...
    axios
      ...
      .catch((error) => {
        dispatch({ type: "FETCH_REVIEW_REQUEST", payload: error.message });
      });
  };
};
...
export const createReview = (reviewData) => {
  return async (dispatch) => {
    ...
    try {
      ...
      dispatch(fetchReviewDetails(response.data.id));
      dispatch(fetchUnitsSuccess(response.data));
    } catch (error) {
      const errorMsg = error.response ? error.response.data.message : error.message;
      dispatch(fetchUnitsFailure(errorMsg));
